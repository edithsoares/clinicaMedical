...
namespace DataAcess
{
    ...
    public class UserDao : ConnectionToSql
    {
        ...
        // Solicitar a senha do usuário do banco de dados
        public string RecoverPassword(string userRequesting)
        {
            using (var connection = GetConnection())
            {
                ...
                {
                    command.Connection = connection;
                    command.CommandText = @"select * from Users where UserName=@user or Email=@mail";
                    ...
                    if (reader.Read() == true)
                    {
                        ...
                        mailServices.sendMail(
                            ...
                            body: "Hi, " + userName + "\nVocê solicitou a recuperação de sua senha.\n" +
                                    ...
                                    "\nNo entanto, pedimos que você altere sua senha imediatamente após entrar no sistema.",
                             destinatarioMail: new List<string> { userMail }
                            );
                        return "Hi, " + userName + "\nVocê solicitou a recuperação de sua senha.\n" +
                                     "Porfavor verifique seu e-mail: " + userMail +
                                     "\nNo entanto, pedimos que você altere sua senha imediatamente após entrar no sistema.";
                    }
                    else
                    {
